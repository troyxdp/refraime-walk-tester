...
MODEL = None
DEVICE = None
CONF_THRES = 0.35 # Perhaps get this from config file?
IOU_THRES = 0.65 # Perhaps get this from config file?
MODEL_NAME = 'rf_intrusion_yolov7_tiny_v2.pt'
COLORS = {
    '0' : (255, 0, 0),
    '1' : (0, 255, 0),
    '2' : (0, 0, 255),
...
def load_yolov7x_model(weights_path):
...
# Methods for loading RTSP data
def get_rtsp_urls(rtsp_yaml):
    with open(rtsp_yaml, 'r') as f:
        data = yaml.load(f, Loader=yaml.SafeLoader)
    # Return camera names (keys of dict) and rtsp urls (values of dict)
    return data.keys(), data.values()

def get_parms(parms_yaml):
...
def contains_save_class(pred, classes_to_save):
    for det in pred:
...
def run_processor(rtsp_url, camera_name, classes_to_save):
    # Global variables
    global MODEL
    global DEVICE
    global CONF_THRES
                ...
                if cmd == StreamCommands.FRAME:
                    ...
                    if val is not None:
                        ...
                        for det in pred[0]:
                            ...
                            color = COLORS[f'{int(det[5])}']
                            label = "{} {:.2f}".format(CLASSES[f'{int(det[5])}'], det[4])
                            # Draw on image
                            img0 = plot_one_box(
                                det[:4], 
                                img0, 
                        ...
                        if len(pred[0]) > 0:
                            ...
                            if contains_save_class(pred[0], classes_to_save):
                                # Save original image
                                cv2.imwrite(
...
def init_program():
    global CAMERA_NAMES, RTSP_URLS, CLASSES_TO_SAVE
    # Get RTSP URLs for testing
    CAMERA_NAMES, RTSP_URLS = get_rtsp_urls('config/streams.yaml')

    # Get parameters for test - model name, confidence threshold, iou threshold, 
    # and classes to save
    parms = get_parms('config/parms.yaml')
    if parms and len(parms) == 4:
        CONF_THRES = parms['conf_thres']
        IOU_THRES = parms['iou_thres']
        MODEL_NAME = parms['model_name']
        CLASSES_TO_SAVE = [int(x) for x in parms['classes_to_save']]
    
    # Load model
    print("Loading model...")
    MODEL = load_yolov7x_model(MODEL_NAME)
    print("Model loaded")
...
def main():
    ...
    # Run tests for each of the RTSP URLs
    logging.info("Running tests for each of the RTSP URLs listed...")
    if RTSP_URLS:
        for camera_name, url in zip(CAMERA_NAMES, RTSP_URLS):
            url = url.strip()
            stop_flag = input(
                f"\nPress enter once you are ready to run walk test for {url}\n"
                "Enter 'stop' to terminate program\n"
            )
            if stop_flag.lower() != 'stop':
                run_processor(url, camera_name, CLASSES_TO_SAVE)
            else:
                break
